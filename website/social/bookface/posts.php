	...
	if(checkValues($_REQUEST['value']))
	{
		...
		mysqli_query("INSERT INTO facebook_posts (post,f_name,userip,date_created) VALUES('".checkValues($_REQUEST['value'])."','99Points','".$userip."','".strtotime(date("Y-m-d H:i:s"))."')");
	...
	elseif($_REQUEST['show_more_post']) // more posting paging
	{
		...
		$check_res = mysqli_query("SELECT * FROM facebook_posts order by p_id desc limit ".$next_records.", 10");
		
		$show_more_button = 0; // button in the end
		
		$check_result = mysqli_num_rows(@$check_res);
		if($check_result > 0)
		{
			$show_more_button = 1;
		}
