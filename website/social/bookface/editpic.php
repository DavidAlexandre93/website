...
$user= $_SESSION['SESS_FIRST_NAME'];
	...
	$image_size= getimagesize($_FILES['image']['tmp_name']);
		...
		if ($image_size==FALSE) {
			...
			$update2=mysqli_query("UPDATE postcomment SET pic = '$location' WHERE commentedby='$user'");
			$update3=mysqli_query("UPDATE message SET picture = '$location' WHERE user='$user'");
			
			/*mysqli_query("UPDATE members SET profImage = '$location' WHERE message_id='$messageid'");

			mysqli_close($con);*/
			if(!$update=mysqli_query("UPDATE members SET profImage = '$location' WHERE member_id='$id'")) {
